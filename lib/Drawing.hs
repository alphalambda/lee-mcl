...
writePoints filename filter =
    do gen <- getStdGen
       let points = filter . randomPoints $ gen
       writeFile filename (concat . map dump $ points)
    where dump (x,y) = show x ++ " " ++ show y ++ "\n"

drawPicture_ generate points =
    P.display (P.InWindow windowtitle (pixelWidth,pixelHeight) windowPosition)
        P.white
        (P.scale s s $ generate points)
    where s = fromIntegral pixelRadius / modelSize

drawPicture :: ([Point] -> P.Picture) -> IO ()
drawPicture generate =
        do gen <- getStdGen
           drawPicture_ generate (randomPoints gen)

drawFile :: String -> ([Point] -> P.Picture) -> IO ()
drawFile filename generate =
    do content <- readFile filename
       let points = parse . words $ content
       drawPicture_ generate points
    where parse [] = []
          parse [x] = []
          parse (x:y:rest) = (float x,float y) : parse rest
...
makeRange = makeRangeWith 1.0

makeRangeWith res size = [-margin,-next..margin]
    where margin = size * modelSize
          next = margin - res * plot_resolution

drawGraphWithOn drawPath xs f = drawPath . map f' $ xs
    where f' x = (x,f x)

drawGraph = drawGraphWithOn drawPath (makeRange 4)
drawThickGraph thickness = drawThickGraphOn thickness (makeRange 4)

drawThickGraphOn thickness = drawGraphWithOn (drawThickPath thickness)
